...
public:
    ...
    {
        static ChatCommandTable creatureRespawnCommandTable =
        {
            { "reload", HandleCRReloadCommand,  SEC_ADMINISTRATOR,   Console::Yes },
            { "info",   HandleCRInfoCommand,    SEC_PLAYER,          Console::No },
        };

        static ChatCommandTable commandTable =
        {
            { "cr", creatureRespawnCommandTable },
            { "boss", creatureRespawnCommandTable }
        };
    ...
    static bool HandleCRInfoCommand(ChatHandler* handler, Optional<uint32> argPage)
    {
        if (!sCreatureRespawnMgr->IsEnable())
        {
            handler->PSendSysMessage("> Module disabled");
        ...
        sCreatureRespawnMgr->SendCreatureRespawnInfo(handler, page);
        return true;
    }
};
